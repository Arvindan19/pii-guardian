...
with tab1:
    ...
    uploaded = st.file_uploader("Drop your CSV here", type=["csv"], key="t1_upload")
    ...
    if uploaded is None:
        ...
        if not entities:
            ...
            if cell_log:
                ...
                with dl1:
                    st.download_button("⬇️ Fake CSV", df_to_csv_bytes(clean_df),
                        f"fake_{uploaded.name}", "text/csv", type="primary", use_container_width=True)
                with dl2:
                    st.download_button("🔑 Lookup Table", df_to_csv_bytes(log_df),
                        f"lookup_{uploaded.name}", "text/csv", use_container_width=True)
                with dl3:
                    st.download_button("📄 Original CSV", df_to_csv_bytes(raw_df),
                        f"original_{uploaded.name}", "text/csv", use_container_width=True)
            else:
...
with tab2:
    ...
    t2_upload = st.file_uploader("Drop your CSV here", type=["csv"], key="t2_upload")

    if t2_upload is None:
        st.info("Upload a CSV to get started.", icon="📂")
    else:
        try:
            t2_raw = pd.read_csv(t2_upload, dtype=str)
        except Exception as e:
            st.error(f"Could not parse CSV: {e}")
            st.stop()
        ...
        if st.button("🎯 Anonymise by Column", type="primary", use_container_width=True):
            ...
            dl1, dl2, dl3 = st.columns(3)
            with dl1:
                st.download_button("⬇️ Fake CSV", df_to_csv_bytes(t2_clean),
                    f"fake_{t2_upload.name}", "text/csv", type="primary", use_container_width=True)
            with dl2:
                st.download_button("🔑 Lookup Table", df_to_csv_bytes(t2_log_df),
                    f"lookup_{t2_upload.name}", "text/csv", use_container_width=True)
            with dl3:
                st.download_button("📄 Original CSV", df_to_csv_bytes(t2_raw),
                    f"original_{t2_upload.name}", "text/csv", use_container_width=True)
...
with tab3:
    ...
    with col_a:
        fake_upload   = st.file_uploader("1️⃣  Analysed fake CSV",  type=["csv"], key="t3_fake")
    with col_b:
        lookup_upload = st.file_uploader("2️⃣  Lookup table",       type=["csv"], key="t3_lookup")

    if fake_upload and lookup_upload:
        try:
            fake_df   = pd.read_csv(fake_upload,   dtype=str)
        ...
        if restored_df is None:
            ...
            st.markdown("---")
            st.download_button(
                "⬇️ Download restored CSV", df_to_csv_bytes(restored_df),
                f"restored_{fake_upload.name}", "text/csv",
